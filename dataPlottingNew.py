...
def exp_force():
    ...
    df = pd.read_csv(filename, usecols=[
        'time', '.wrench.force.x', '.wrench.force.y', '.wrench.force.z'])
    ...
    df = df.rename(columns={
        '.wrench.force.x': 'Fx', '.wrench.force.y': 'Fy',
        '.wrench.force.z': 'Fz', 'time': 'Time'})
...
def sim_force():
    ...
    df = pd.read_csv(filename, header=1, usecols=[
        'Time', 'wheel.fx', 'wheel.fy', 'wheel.fz'])
    ...
    df = df.rename(columns={
        'wheel.fx': 'Fx', 'wheel.fy': 'Fy', 'wheel.fz': 'Fz'})
    return df
...
def plot_data(df_exp=None, df_sim=None):
    ...
    ax[0].set(
        xlabel='Time', ylabel=plot_value, title=f'{plot_value}: SR{SR}',
        autoscale_on=True)
    """ax[0].plot(
        'Time', 'Fx', data=df_exp['force'], linestyle='-',
        label='Experiment')"""
    ax[0].plot(
        'Time', plot_value, data=df_sim['force'], linestyle='-',
        label='Simulation')

    ax[1].set(
        xlabel='Time', ylabel='Sinkage',
        title=f'Sinkage: SR{SR}', autoscale_on=True)
    ax[1].plot(
        'Time', 'Sinkage', data=df_exp['sinkage'], linestyle='-',
        label='Experiment')
    """ax[1].plot(
